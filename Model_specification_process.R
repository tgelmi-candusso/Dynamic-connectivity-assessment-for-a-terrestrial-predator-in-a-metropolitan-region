...
#### Include steplength and angle in the model#####
#try to include it in the habmodel, just as an extra parameter.
step_length
turn

daymodel34_sa<-glmmTMB(used ~ -1 + NDVI_end + POP_end + BUP_end + 
                         DT_LT_end + DT_MT_end + DT_HT_end + DT_NT_end + DT_PS_end + 
                         POP_end:day+DT_PS_end:day +NDVI_end:day+DT_HT_end:day +DT_NT_end:day +DT_LT_end:day +
                         step_length + 
                         turn +
                         (1|animal) + 
                         (0+NDVI_end|animal) + 
                         ...
                         (0+DT_NT_end|animal) + 
                         (0+DT_PS_end|animal) +
                         (0+POP_end|animal) + (0+DT_PS_end:day |animal) + (0+NDVI_end:day|animal) + (0+DT_HT_end:day |animal) + (0+DT_NT_end:day |animal) + (0+DT_LT_end:day |animal) + 
                         (0+step_length|animal) + (0+turn:day |animal) +
                         (1|step), data = coyote_steps, 
                       map = list(theta = factor(c(1:33, NA))),
                       start = list(theta = c(rep(0, 33), log(1e3))),
                       family = poisson)
saveRDS(daymodel34_sa, "daymodel34_sa.rds")
capture.output(summary(daymodel34_sa),file="daymodel34_inc_Step_length_and_angle.doc")

model_sa<-glmmTMB(used ~ -1 + NDVI_end + POP_end + BUP_end + 
                    DT_LT_end + DT_MT_end + DT_HT_end + DT_NT_end + DT_PS_end +
                    step_length:NDVI_end + step_length:POP_end + step_length:BUP_end + step_length:DT_LT_end + step_length:DT_MT_end + step_length:DT_HT_end + step_length:DT_NT_end + step_length:DT_PS_end + 
                    turn:NDVI_end + turn:POP_end + turn:BUP_end + turn:DT_LT_end + turn:DT_MT_end + turn:DT_HT_end + turn:DT_NT_end + turn:DT_PS_end + 
                    (1|animal) +  
                    (0+NDVI_end|animal) + 
                    (0+POP_end|animal) +
                    (0+BUP_end|animal) + 
                    ...
                    (0+DT_NT_end|animal) + 
                    (0+DT_PS_end|animal) +
                    (0+step_length:NDVI_end|animal) + 
                    (0+step_length:POP_end|animal) +
                    (0+step_length:BUP_end|animal) + 
                    (0+step_length:DT_LT_end|animal) + 
                    (0+step_length:DT_MT_end|animal) + 
                    (0+step_length:DT_HT_end|animal) + 
                    (0+step_length:DT_NT_end|animal) + 
                    (0+step_length:DT_PS_end|animal) +
                    (0+turn:NDVI_end|animal) + 
                    (0+turn:POP_end|animal) +
                    (0+turn:BUP_end|animal) + 
                    (0+turn:DT_LT_end|animal) + 
                    (0+turn:DT_MT_end|animal) + 
                    (0+turn:DT_HT_end|animal) + 
                    (0+turn:DT_NT_end|animal) + 
                    (0+turn:DT_PS_end|animal) +
                    (1|step), data = coyote_steps, 
                  map = list(theta = factor(c(1:25, NA))),
                  start = list(theta = c(rep(0, 25), log(1e3))),
                  family = poisson)
saveRDS(model_sa, "model_sa.rds")
capture.output(summary(model_sa),file="model_inc_Step_length_and_angle.doc")
